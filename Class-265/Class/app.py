...
def upload_image():
    ...
    with open(os.path.join("./Class-265/Class/static/", filename), "wb") as f:
        f.write(file_data)
    # ends here
...
def make_grayscale(input_file):
    image_array = np.fromstring(input_file, dtype="uint8")
    print("Image Array:", image_array)

    decode = cv2.imdecode(image_array, cv2.IMREAD_UNCHANGED)
    print("Decode Image:", decode)

    grayscale = cv2.cvtColor(decode, cv2.COLOR_RGB2GRAY)
    status, output_image = cv2.imencode(".PNG", grayscale)
    print("Status:", status)
    return output_image
