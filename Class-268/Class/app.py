...
def water_color_effect(decode_array_to_img):
    rgb_effect_img = cv2.stylization(decode_array_to_img, sigma_s=60, sigma_r=0.6)
    status, output_img = cv2.imencode(".PNG", rgb_effect_img)
...
def invert(decode_array_to_img):
    rgb_effect_img = cv2.bitwise_not(decode_array_to_img)
    status, output_img = cv2.imencode(".PNG", rgb_effect_img)
...
def hdr(decode_array_to_img):
    rgb_effect_img = cv2.detailEnhance(decode_array_to_img, sigma_s=12, sigma_r=0.15)
    status, output_img = cv2.imencode(".PNG", rgb_effect_img)
